...
Startup and file-watcher reconciliation both register jobs through `RuntimeCronService`. That service passes `missedExecutionTolerance: 24h` (and a job `name`) into node-cron, and polls wall clock every 30s for due slots. Docker Desktop often pauses the Linux VM even while the Mac is awake, which freezes node-cron’s long next-run `setTimeout`; the poll runs at most one eligible late slot (capped by the gap to the next fire) as soon as the process is ticking again, then re-arms the task. Missed slots beyond that window are not backfilled. Process restart / container recreate still starts from the next future fire only. The dedicated scheduler process honors `ENABLE_SCHEDULER`: when disabled it stays idle until shutdown instead of scheduling jobs.
...
| Late wake after host sleep | 30s wall-clock due-slot poll + `missedExecutionTolerance: 24h` in `RuntimeCronService` | Docker Desktop VM pauses freeze node-cron timers even when the Mac is awake; poll runs one late slot when the process ticks again. Not a multi-day or restart backfill. |
